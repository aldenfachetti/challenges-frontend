...
// '1 00000 and thus no binary gaps.

// Write an efficient algorithm for the following assumptions:
//         N is an integer within the range [1..2,147,483,647j.
...
function solution(N) {
  ...
  let binaryGap = 0;
  let binaryGaps = [];
  let binaryGapsLength = 0;
  let binaryGapsLengths = [];
  let binaryGapsLengthsSorted = [];
  ...
  for (let i = 0; i < binaryGaps.length; i++) {
    binaryGapsLength = binaryGaps[i];
    binaryGapsLengths.push(binaryGapsLength);
  }

  binaryGapsLengthsSorted = binaryGapsLengths.sort(function (a, b) {
    return b - a;
  });

  if (binaryGapsLengthsSorted.length == 0) {
    return 0;
  } else {
    return binaryGapsLengthsSorted[0];
  }
}
